...
export const clearCart = createAsyncThunk("cart/clearCart", async () => {
  const { data } = await axios.delete(`/cart/clear-cart/`);
  return data;
});
...
export const removeItemFromCart = createAsyncThunk(
  "cart/removeItem",
  async (id) => {
    const { data } = await axios.delete(`/cart/items/${id}/`);
    return data;
  },
);
...
export const checkout = createAsyncThunk("cart/checkout", async (token) => {
    ...
    getTotal: (state) => {
      state.total = state.cartItems.reduce((a, b) => {
        return (
          Math.round(
            (a +
              (b.quantity * Math.round(Number(b.product_detail.price) * 100)) /
                100) *
              100,
          ) / 100
        );
